return {
  'lewis6991/gitsigns.nvim', opts = {
    ...
    on_attach = function(bufnr)
      ...
      map('n', '<leader>1', gs.next_hunk, {desc = 'Go To Next Git Hunk'})

      map('n', '<F2>',      gs.prev_hunk, {desc = 'Go To Previous Git Hunk'})
      map('n', '<leader>2', gs.prev_hunk, {desc = 'Go To Previous Git Hunk'})

      map('n', '<F3>',      gs.preview_hunk_inline, {desc = 'Preview Git Diff'})
      map('n', '<leader>3', gs.preview_hunk_inline, {desc = 'Preview Git Diff'})

      map('n', '<F4>',      gs.stage_hunk, {desc = 'Stage Current Git Hunk'})
      map('n', '<leader>4', gs.stage_hunk, {desc = 'Stage Current Git Hunk'})

      map('v', '<F4>',      function() gs.stage_hunk {vim.fn.line('.'), vim.fn.line('v')} end, {desc = 'Stage Selected Git Hunk'})
      map('v', '<leader>4', function() gs.stage_hunk {vim.fn.line('.'), vim.fn.line('v')} end, {desc = 'Stage Selected Git Hunk'})
